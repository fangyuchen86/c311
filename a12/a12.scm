...
(define !-o
  (lambda (gamma e t)
    (matche `(,gamma ,e ,t)
      ...
      ;; Add your own definitions for variable, lambda, and
      ;; application expressions here.      
      [(,gamma (var ,x) ,t)
       (lookupo x gamma t)]
      [(,gamma (lambda (,x) ,body) (-> ,t-rand ,t-body))
       (!-o `((,x . ,t-rand) . ,gamma) body t-body)]
      [(,gamma (app ,rator ,rand) ,t)
       (exist (t-rand)
              (!-o gamma rand t-rand)
              (!-o gamma rator `(-> ,t-rand ,t))
              )]
      )))
